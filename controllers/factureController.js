...
exports.getFacture = (req, res) => {
    const { page = 1, pageSize = 10, id_client } = req.query;
    ...
    const q = `
        ...
        ${id_client ? 'WHERE factures.id_client = ?' : ''}
        GROUP BY factures.id_facture
        LIMIT ?, ?
    `;
    
    const queryParams = id_client ? [id_client, offset, parseInt(pageSize)] : [offset, parseInt(pageSize)];
...
exports.postFacture = (req, res) => {
    const { id_client, date_facture, total, details, commentaire } = req.body;

    db.query('INSERT INTO factures (id_client, date_facture, total, commentaire) VALUES (?, ?, ?, ?)', [id_client, date_facture, total, commentaire], (err, result) => {
        if (err) throw err;
